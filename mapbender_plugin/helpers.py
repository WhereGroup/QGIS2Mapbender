...
def upload_project_zip_file(c, source_project_zip_dir_path: str,
                            server_qgis_projects_folder_rel_path: str, qgis_project_folder_name: str) -> bool:
    """
...
def upload(c, projects_path, qgis_project_folder_name, source_project_dir_path,
    ...
    unzip_project_folder_on_server(c, qgis_project_folder_name, projects_path)


def unzip_project_folder_on_server(c, qgis_project_folder_name: str,
    ...
    """
    c.run(f'cd ..; cd {projects_path}/; unzip {qgis_project_folder_name}.zip;')
    QgsMessageLog.logMessage("Unzipping files on the server...", TAG, level=Qgis.Info)
    c.run(f'cd ..; cd /data/qgis-projects/; rm {qgis_project_folder_name}.zip;')
